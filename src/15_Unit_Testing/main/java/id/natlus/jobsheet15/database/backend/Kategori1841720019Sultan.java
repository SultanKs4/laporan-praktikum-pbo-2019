...
public class Kategori1841720019Sultan {
    ...
    public ArrayList<Kategori1841720019Sultan> getByNamaAndKeteranganSultan(String nama, String keterangan) {
        ArrayList<Kategori1841720019Sultan> ListKategori = new ArrayList<>();
        ResultSet rs;

        if ((nama.trim().length() > 0) && (keterangan.trim().length() > 0)) {
            rs = DBHelper1841720019Sultan.selectQuerySultan("SELECT * FROM kategori Where nama = '"
                    + nama + "' and keterangan = '" + keterangan + "'");
        } else if ((nama.trim().length() > 0) && (keterangan.trim().length() == 0)) {
            rs = DBHelper1841720019Sultan.selectQuerySultan("SELECT * FROM kategori Where nama = '"
                    + nama + "'");
        } else {
            rs = DBHelper1841720019Sultan.selectQuerySultan("SELECT * FROM kategori Where keterangan'" + keterangan + "'  ");
        }
